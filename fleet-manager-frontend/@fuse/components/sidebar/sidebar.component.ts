...
import {flatMap, take, takeLast, takeUntil} from 'rxjs/operators';
...
export class FuseSidebarComponent implements OnInit, OnDestroy {
    ...
    private _triggerClosed: boolean = false;
    /**
     * Constructor
     *
     ...
     */
    open(): void {
        if (this.opened && this.isLockedOpen && !this._triggerClosed) {
            return;
        }
     ...
     */
    close(): void {
        if (!this.opened || !this._triggerClosed) {
            return;
        }
     ...
     * @private
     */
    private _subscribeToMediaChanges() {
